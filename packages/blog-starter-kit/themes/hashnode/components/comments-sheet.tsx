    ...
    <DialogPrimitive.Root open>
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay
          onClick={hideSheet}
          className={twJoin(
            'fixed inset-0 z-50 bg-neutral-900 opacity-0 transition-opacity ease-out dark:bg-slate-600',
            isMounted && 'opacity-50 duration-300',
          )}
        />
        <DialogPrimitive.Content
          className={twJoin(
            'fixed top-0 bottom-0 right-0 z-50 w-full transform-gpu overflow-auto border-slate-200 bg-white shadow-2xl transition-transform duration-300 ease-out dark:border-l dark:border-slate-700 dark:bg-neutral-900 md:w-108',
